        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-semibold">Tokens de acesso</h1>
                        <p className="text-sm text-muted-foreground">
                            Gere tokens permanentes para integrar com a API de assets.
                        </p>
                    </div>
                    ...
                    <table className="min-w-full divide-y divide-border/70 text-sm">
                        <thead className="bg-muted/50">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left font-medium text-foreground">Prévia</th>
                                <th className="px-4 py-3 text-left font-medium text-foreground">Criado em</th>
                                <th className="px-4 py-3 text-left font-medium text-foreground">Último uso</th>
                        ...
                        <tbody className="divide-y divide-border/70 bg-background">
                            {tokens.length === 0 && (
                                <tr>
                                    <td className="px-4 py-6 text-center text-sm text-muted-foreground" colSpan={5}>
                                        Nenhum token cadastrado até o momento.
                                    </td>
                                </tr>
                            )}
                            ...
                            {tokens.map((token) => (
                                <tr
                                    key={token.id}
                                    className={highlightedId === token.id ? 'bg-primary/5' : undefined}
                                >
                                    ...
                                    <td className="px-4 py-3 align-middle text-muted-foreground">
                                        {token.last_used_at ? formatDateTime(token.last_used_at) : 'Nunca utilizado'}
                                    </td>
            ...
            <Dialog open={isCreateOpen} onOpenChange={closeCreateDialog}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Defina um nome para identificar o token (opcional).
                        </DialogDescription>
                    </DialogHeader>
            ...
            <Dialog open={isPlainTokenOpen} onOpenChange={setIsPlainTokenOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Token criado</DialogTitle>
                        <DialogDescription>
                            Guarde este token — ele não será exibido novamente.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between gap-3">
                            <Button type="button" variant="outline" onClick={handleCopyToken} className="flex-1">
                                <Copy className="mr-2 size-4" />
                                Copiar
                            </Button>
                            {copiedToken && copiedToken === generatedToken && (
                    ...
                    <DialogFooter>
                        <Button type="button" onClick={() => setIsPlainTokenOpen(false)}>
                            Fechar
                        </Button>
                    </DialogFooter>
                </DialogContent>
